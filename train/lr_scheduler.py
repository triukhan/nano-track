...
LR = {
    'TYPE': 'log',
    'KWARGS': {'start_lr': 0.005, 'end_lr': 0.0005 }
}
WARMUP = True
LR_WARMUP = {
    'TYPE': 'step',
...
def _build_warm_up_scheduler(optimizer, epochs=50, last_epoch=-1):
    warmup_epoch = LR_WARMUP['EPOCH']
    sc1 = _build_lr_scheduler(optimizer, LR_WARMUP,
                              warmup_epoch, last_epoch)
    sc2 = _build_lr_scheduler(optimizer, LR,
...
def build_lr_scheduler(optimizer, epochs=50, last_epoch=-1):
    if WARMUP:
        ...
        return _build_lr_scheduler(optimizer, LR,
                                   epochs, last_epoch)
